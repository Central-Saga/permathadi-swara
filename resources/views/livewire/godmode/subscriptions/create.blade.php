    ...
    <flux:card>
        <form wire:submit="store" class="space-y-6">
            <div>
                ...
                <flux:select wire:model="anggota_id" name="anggota_id" variant="listbox" searchable
                    placeholder="{{ __('Pilih anggota...') }}" class="mt-2" required>
            ...
            <div>
                ...
                <flux:select wire:model.live="layanan_id" name="layanan_id" variant="listbox" searchable
                    placeholder="{{ __('Pilih layanan...') }}" class="mt-2" required
                    x-on:change="$wire.calculateEndDate()">
                    @forelse ($this->getLayanans as $layanan)
                    <flux:select.option value="{{ $layanan->id }}">
                        {{ $layanan->name }} ({{ $layanan->duration }} hari)
                    </flux:select.option>
                    @empty
            ...
            <div>
                ...
                <flux:date-picker wire:model.live="start_date" name="start_date" min="{{ now()->format('Y-m-d') }}"
                    required x-on:change="$wire.calculateEndDate()">
                    <x-slot name="trigger">
