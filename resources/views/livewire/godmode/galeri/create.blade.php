...
use function Livewire\Volt\{layout, title, state, action, uses};

uses(WithFileUploads::class);

layout('components.layouts.admin');
title(fn () => __('Tambah Galeri'));
...
$store = action(function () {
    ...
    if (!empty($this->images)) {
        foreach ($this->images as $image) {
            $galeri->addMedia($image->getRealPath())
    ...
    $this->dispatch('toast', message: __('Galeri berhasil dibuat.'), variant: 'success');
...
<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Tambah Galeri') }}</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Buat galeri dokumentasi baru') }}</p>
        </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        ...
                        @endif
                        <flux:file-upload wire:model="images" label="{{ __('Upload Gambar') }}" multiple>
