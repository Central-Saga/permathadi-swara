...
$generateSlug = action(function () {
    ...
    $originalNameSlug = Str::slug($this->originalName);
    
    // Only auto-update if slug hasn't been manually edited
    // Check if current slug matches what would be generated from original name
    if (!$this->slugManuallyEdited || $this->slug === $originalNameSlug || empty($this->slug)) {
        $baseSlug = $expectedSlug;
        $slug = $baseSlug;
        $counter = 1;
        
        // Check if slug exists (excluding current layanan) and make it unique
        while (Layanan::where('slug', $slug)->where('id', '!=', $this->layanan->id)->exists()) {
...
$update = action(function () {
    ...
    if (empty($validated['slug']) && !empty($validated['name'])) {
        ...
        $counter = 1;
        while (Layanan::where('slug', $validated['slug'])->where('id', '!=', $this->layanan->id)->exists()) {
    ...
    // Replace cover image if new image uploaded
    if ($this->cover_image) {
        // Delete old image
        $this->layanan->clearMediaCollection('layanan_cover');
        
        // Add new image
        $this->layanan->addMedia($this->cover_image->getRealPath())
            ->usingName($validated['name'])
            ->usingFileName($this->cover_image->getClientOriginalName())
            ->toMediaCollection('layanan_cover');
    }

    $this->dispatch('toast', message: __('Layanan berhasil diupdate.'), variant: 'success');
...
<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Edit Layanan') }}</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('Ubah informasi layanan') }}</p>
        </div>
    ...
    <flux:card>
        <form wire:submit="update" class="space-y-6">
            <div class="space-y-4">
                <div x-data>
                    <flux:input wire:model.live.debounce.300ms="name" name="name" :label="__('Nama Layanan')" type="text"
                        required autofocus 
                        x-on:input.debounce.300ms="$wire.generateSlug()" />
                </div>
                ...
                <div x-data class="mt-4">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        {{ __('Slug akan otomatis dibuat dari nama layanan dan dijamin unique. Anda bisa mengubahnya manual jika diperlukan.') }}
                    </p>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        ...
                        @elseif ($layanan->getFirstMediaUrl('layanan_cover'))
                        <div class="mb-4">
                        ...
                        <flux:file-upload wire:model="cover_image" label="{{ __('Upload Gambar Cover') }}">
                            <flux:file-upload.dropzone
                                heading="{{ __('Drop file atau klik untuk memilih') }}"
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {{ __('Format: JPG, PNG, GIF. Maksimal 5MB. Upload gambar baru untuk mengganti gambar lama.') }}
                        </p>
                    </div>
                    @error('cover_image')
