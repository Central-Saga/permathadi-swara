...
<div class="bg-white py-24 sm:py-32 dark:bg-gray-900" data-gsap="program-section" data-lazy-section="program">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        <div class="mx-auto max-w-2xl lg:text-center" data-gsap="program-heading">
            <h2 class="text-base/7 font-semibold text-orange-600 dark:text-orange-400">Program Kami</h2>
        ...
        <div class="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-none">
            <div class="grid max-w-xl grid-cols-1 gap-8 lg:max-w-none lg:grid-cols-3">
                @forelse ($layanans as $index => $layanan)
                    <div class="flex flex-col overflow-hidden rounded-2xl bg-gray-50 shadow-sm ring-1 ring-gray-900/5 transition-all hover:shadow-md dark:bg-white/5 dark:ring-white/10" data-gsap="program-card">
                        @if ($layanan->getFirstMediaUrl('layanan_cover'))
                            <div class="aspect-[16/9] w-full overflow-hidden">
                                <img src="{{ $layanan->getFirstMediaUrl('layanan_cover') }}" 
                                    alt="{{ $layanan->name }}"
                                    class="h-full w-full object-cover transition-transform duration-300 hover:scale-105" />
                            </div>
                        @else
                            <div class="aspect-[16/9] w-full bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center">
                                <svg viewBox="0 0 20 20" fill="currentColor" class="size-12 text-white/50">
                                    <path fill-rule="evenodd" d="M1 5.25A2.25 2.25 0 013.25 3h13.5A2.25 2.25 0 0119 5.25v9.5A2.25 2.25 0 0116.75 17H3.25A2.25 2.25 0 011 14.75v-9.5zm1.5 0v9.5a.75.75 0 00.75.75h13.5a.75.75 0 00.75-.75v-9.5a.75.75 0 00-.75-.75H3.25a.75.75 0 00-.75.75zm16.5 0A2.25 2.25 0 0016.75 3H3.25A2.25 2.25 0 001 5.25v9.5A2.25 2.25 0 003.25 17h13.5A2.25 2.25 0 0019 14.75v-9.5z" clip-rule="evenodd" />
                                </svg>
                            </div>
                        @endif
                ...
                @empty
                    <div class="col-span-full text-center py-12">
                        ...
                        <p class="mt-4 text-sm text-gray-600 dark:text-gray-400">Belum ada program yang tersedia saat ini.</p>
                    </div>
                @endforelse
            </div>
        </div>
