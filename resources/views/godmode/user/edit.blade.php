<x-layouts.app :title="__('Edit User')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <flux:card>
            <form action="{{ route('godmode.users.update', $user) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <flux:input name="password" :label="__('Password Baru')" type="password" viewable
                    :error="$errors->first('password')"
                    :placeholder="__('Kosongkan jika tidak ingin mengubah password')" />

                <flux:input name="password_confirmation" :label="__('Konfirmasi Password Baru')" type="password"
